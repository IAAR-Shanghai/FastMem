...
class origin_Model(LlamaPreTrainedModel):
    ...
    def forward(self,
        ...
        if epoch == 0:
                ...
                if mode == 'train':
                    if idx == number_of_layers - self.train_layer_num - 1:
                        self.last_input_dic = {
                            ...
                            'attention_mask': causal_mask.detach().clone(),
            ...
            past_key_values = DynamicCache.get_layer_cache(self.last_input_dic['past_key_values'], number_of_layers - self.train_layer_num, mode=mode)
            for train_idx in range(number_of_layers - self.train_layer_num, number_of_layers):
                layer_outputs = self.layers[train_idx](
                    ...
                    cache_position=self.last_input_dic['cache_position'],
                )
                hidden_states = layer_outputs[0]
...
class LlamaModel_ours(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = origin_Model(config)
        self.model.set_input_dic()
        self.vocab_size = config.vocab_size
    ...
    def set_any_layer_num(self, layer_num, label=0):
        ...
        print(self.model.train_layer_num, len(self.model.layers))
    ...
    def forward(
    ...
    ) -> Union[Tuple, CausalLMOutputWithPast_new]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        if self.mode=='test' and choose_dola:
...
if __name__ == "__main__":
    model = LlamaModel_ours.from_pretrained(
        "/mnt/huggingface/models/Llama-2-7b-chat-hf"
    ).cuda()
    tokenizer = transformers.AutoTokenizer.from_pretrained("/mnt/huggingface/models/Llama-2-7b-chat-hf", trust_remote_code=True)
    input_ids = tokenizer.encode('Lalo Schifrin ', return_tensors="pt").cuda()
    print(input_ids)
    past_key_values = None
    with torch.no_grad():
        model_inputs = model.prepare_inputs_for_generation(input_ids, past_key_values=past_key_values)
        outputs = model(**model_inputs, output_hidden_states=True, epoch=0)

        outputs = model(**model_inputs, output_hidden_states=False, epoch=1)
        print(outputs)
